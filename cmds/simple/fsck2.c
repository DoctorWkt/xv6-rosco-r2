...
int nblk;
{
  ...
  bitmap = (unsigned *) alloc((unsigned) nblk, BLOCK_SIZE);
  *bitmap |= 1;
  return(bitmap);
}
...
zone_nr zno;
{
  ...
  do {
	...
	for (dp = dirblk; dp < &dirblk[CDIRECT]; dp++) {
		...
		if (dp->d_inum != NO_ENTRY && !chkentry(ino, pos, dp))dirty= 1;
...
char *f, **clist, **ilist, **zlist;
{
  ...
  initbitmap(zmap, (bit_nr) sb.s_zones - (bit_nr) FIRST + 1, N_ZMAP);
